...
1. **start** — validate inputs → ensure schema → INSERT `discovery_jobs` row with status `running` → kick off `runDiscoveryProduction(...)` non-awaited (fire-and-forget) using a `RunRecorder` wrapper that tags every `discovery_runs` row and finding with the `job_id` → return `{ jobId }`.
2. **status** — read `discovery_jobs` row + count `discovery_runs` by `job_id` → return `{ status, startedAt, finishedAt?, completedSourceCount, requestedSources?, error? }`. The total source count is taken from the jurisdiction registry filtered by `requestedSources`/`requestedJurisdiction`.
3. **result** — read `discovery_jobs` row (must be `completed`) → read `discovery_runs` and `findings` by `job_id` → re-assemble a `DiscoveryReport`-shaped response (with the same per-source summary metadata).
4. **failure mode** — the fire-and-forget wrapper catches all errors and writes them to `discovery_jobs.error_*` + flips status to `failed`. The Cloud Run container restart risk is acknowledged: a job that was `running` at restart time is effectively orphaned. Operationally we'll tolerate it (job rows get a stale-detection rule in a later iteration); for this branch we don't add a watchdog.
